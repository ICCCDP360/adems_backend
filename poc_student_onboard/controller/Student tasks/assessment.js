...
exports.GetAssessment = async (req, res) => {
  ...
  let langtype = req.query.lang_type;
  // console.log(langtype);
  try {
    // get all data
    assessment.find({lang_type:langtype || 'english'}).exec(function (err, users) {
      if (users) {
        // console.log(users);
        var dataSet = [];
        for (let index = 0; index < users.length; index++) {
          ...
          let data = {
            ...
            status_percentage: element.status_percentage,
            lang_type: element.lang_type,
            //lang: element.lang,
